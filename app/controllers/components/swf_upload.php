...
class SwfUploadComponent extends Object {
	...
	function write() {
		...
		if (!$folder) {
			$this->setError(1500, __('File system save failed.'), __('Could not create requested directory: ') . $this->uploadpath);
			$this->log(  'No se pudo crear el directorio configurado' );
		} else {
			if (!$this->overwrite) {
			...
			if (!($moved = move_uploaded_file($this->params['form']['Filedata']['tmp_name'], $this->uploadpath . $this->filename))) {
				$this->setError(1000, __('File system save failed.'));
				$this->log(  'Error al mover el archivo'.$this->uploadpath . $this->filename );
			}
		}
		return $moved;
	}
	...
	function validate() {
		...
		if (!$post_ok){
			$this->setError(2000, __('Validation failed.'), __('Expected file upload field to be named "Filedata."'));
		}
		if ($upload_error){
			$this->setError(2500, __('Validation failed.'), $this->getUploadErrorMessage($upload_error));
		}
	...
	function getUploadErrorMessage($err) {
		...
		switch ($err) {
			...
			case UPLOAD_ERR_INI_SIZE:
				$msg = __('The uploaded file exceeds the upload_max_filesize directive (').ini_get('upload_max_filesize').') en php.ini.';
				break;
			...
			case UPLOAD_ERR_NO_TMP_DIR:
				$msg = __('The remote server has no temporary folder for file uploads.');
				break;
			case UPLOAD_ERR_CANT_WRITE:
				$msg = __('Failed to write file to disk.');
				break;
			default:
				$msg = __('Unknown File Error. Check php.ini settings.');
		}
